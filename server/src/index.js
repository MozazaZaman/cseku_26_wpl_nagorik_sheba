...
app.get('/api/stats', (_req, res) => {
  ...
  res.json({
    ...
    in_process: count("SELECT COUNT(*) n FROM complaints WHERE status IN ('assigned','in_process')"),
    in_review: count("SELECT COUNT(*) n FROM complaints WHERE status IN ('verified','under_field_review','mid_review','mayor_review','approved')"),
  ...
  const rows = req.auth.role === 'staff'
    ? db.prepare(
        `SELECT * FROM notifications
         WHERE recipient_type = 'staff' AND user_id = ? ORDER BY created_at DESC LIMIT 50`
      ).all(req.auth.id)
    : db.prepare(
        `SELECT * FROM notifications
         WHERE recipient_type = 'citizen' AND user_id = ? ORDER BY created_at DESC LIMIT 50`
      ).all(req.auth.id);
...
app.patch('/api/my/notifications/read-all', requireAuth, (req, res) => {
  if (req.auth.role === 'citizen') {
    db.prepare("UPDATE notifications SET is_read = 1 WHERE user_id = ? AND recipient_type = 'citizen'").run(req.auth.id);
  } else {
    db.prepare("UPDATE notifications SET is_read = 1 WHERE user_id = ? AND recipient_type = 'staff'").run(req.auth.id);
  }
  res.json({ ok: true });
});
