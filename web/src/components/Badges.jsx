...
export function StatusBadge({ status }) {
  ...
  const map = {
    ...
    verified: 'bg-sky-500/15 text-sky-300 border-sky-400/30',
    junior_review: 'bg-sky-500/15 text-sky-300 border-sky-400/30',
    senior_review: 'bg-violet-500/15 text-violet-300 border-violet-400/30',
    mayor_review: 'bg-violet-500/15 text-violet-300 border-violet-400/30',
    approved: 'bg-mint/15 text-mint border-mint/40',
    wit_review: 'bg-mint/15 text-mint border-mint/40',
  ...
  const dots = {
    submitted: 'bg-sky-400', verified: 'bg-sky-400', junior_review: 'bg-sky-400',
    senior_review: 'bg-violet-400', mayor_review: 'bg-violet-400', approved: 'bg-mint',
    wit_review: 'bg-mint', assigned: 'bg-amber-400', in_process: 'bg-amber-400',
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold ${m}`}>
      <span className={`h-1.5 w-1.5 rounded-full ${dots[status] || dots.submitted} ${['assigned', 'in_process'].includes(status) ? 'animate-pulse' : ''}`} />
