...
import LifecycleCard from '../components/LifecycleCard.jsx';
...
export default function ComplaintDetail() {
  ...
  // WIT needs the executor list for the assignment action
  useEffect(() => {
    if (data?.my_actions?.includes('wit_assign')) {
      api.get('/complaints/staff/executors')
        .then(({ data: d }) => setExecutors(d.executors))
        .catch(() => {});
    }
  }, [data?.my_actions]);
  ...
  const locked = ['assigned', 'in_process'].includes(c.status);
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[1.35fr_1fr]">
        <div className="space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass p-6">
            ...
            <div className="mt-5 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] p-4">
              ...
              {!c.is_mine && ['submitted', 'verified', 'junior_review', 'senior_review', 'mayor_review'].includes(c.status) && (
                <button onClick={vote} disabled={busy || c.voted_by_me}
                  className={`btn-primary !py-2.5 ${c.voted_by_me ? '!bg-none !bg-mint/20 !text-mint' : ''}`}>
          ...
          {user?.role === 'staff' && data.my_actions?.length > 0 && (
            <WorkflowActions complaint={c} myActions={data.my_actions} executors={executors} onDone={load} />
          )}
          <LifecycleCard complaint={c} workflow={data.workflow} />
        ...
        <div className="space-y-6">
          ...
          <div className="glass space-y-3 p-5 text-sm">
            ...
            <div className="flex justify-between"><span className="text-slate-500">{t('det.staff')}</span><span className="text-slate-300">{c.executor_name || c.staff_name || t('det.awaitingAssignment')}</span></div>
