...
function AppearanceSwitch() {
  ...
  return (
    <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0.5}
      className="mt-8 inline-flex items-center gap-3 rounded-2xl glass px-4 py-3">
      <span className="text-xs font-semibold uppercase tracking-widest text-slate-500">{t('theme.label')}</span>
      <div className="flex gap-1.5">
        {options.map((o) => (
          <button
            key={o.key}
            onClick={() => setTheme(o.key)}
            className={`chip text-xs ${theme === o.key ? 'chip-active' : 'chip-idle'}`}
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6">
      ...
      <motion.section initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
        className="land-cta-panel relative mb-4 overflow-hidden rounded-3xl border border-white/10 p-12 text-center">
